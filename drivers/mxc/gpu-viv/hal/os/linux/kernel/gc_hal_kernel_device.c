    ...
    )
{
    ...
    if (IrqLine != -1) {
        device->clk_3d_core = devm_clk_get(&pdev->dev, "gpu3d_clk");
        if (IS_ERR(device->clk_3d_core)) {
            IrqLine = -1;
            device->clk_3d_core = NULL;
            dev_err(&pdev->dev, "clk_get gpu3d_clk failed, disable 3d!\n");
        } else
            clk_prepare_enable(device->clk_3d_core);
    }

#if 0
    if (IrqLine != -1) {
        device->clk_3d_shader = devm_clk_get(&pdev->dev, "gpu3d_shader_clk");
        if (IS_ERR(device->clk_3d_shader)) {
            IrqLine = -1;
            device->clk_3d_shader = NULL;
            dev_err(&pdev->dev, "clk_get gpu3d_shader_clk failed, disable 3d!\n");
        } else
            clk_prepare_enable(device->clk_3d_shader);
    }
#endif

    if ((IrqLine2D != -1) || (IrqLineVG != -1)) {
        device->clk_2d_core = devm_clk_get(&pdev->dev, "gpu2d_clk");
        if (IS_ERR(device->clk_2d_core)) {
            IrqLine2D = -1;
            IrqLineVG = -1;
            device->clk_2d_core = NULL;
            dev_err(&pdev->dev, "galcore: clk_get 2d clock failed, disable 2d/vg!\n");
        } else
            clk_prepare_enable(device->clk_2d_core);
    }
