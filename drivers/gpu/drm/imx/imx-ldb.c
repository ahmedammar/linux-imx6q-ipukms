...
static enum drm_connector_status imx_ldb_connector_detect(struct drm_connector *connector,
		bool force)
{
...
static struct drm_encoder *imx_ldb_connector_best_encoder(struct drm_connector *connector)
{
	struct imx_ldb_channel *imx_ldb_ch = con_to_imx_ldb_ch(connector);
...
//	adjusted_mode->clock = clk_round_rate(imx_ldb_ch->clk_pll, adjusted_mode->clock * 1000) / 1000;
...
static void imx_ldb_encoder_prepare(struct drm_encoder *encoder)
{
	...
	printk("%s: now: %ld want: %ld\n", __func__,
			clk_get_rate(imx_ldb_ch->clk_pll), mode->clock * 1000 * 7);
	clk_set_rate(imx_ldb_ch->clk_pll, mode->clock * 1000 * 7);

	printk("%s after: %ld\n", __func__, clk_get_rate(imx_ldb_ch->clk_pll));
...
static int imx_ldb_register(struct imx_ldb_channel *imx_ldb_ch, void *edid, int edid_len,
		u32 crtcs)
{
	int ret;
	struct imx_ldb *ldb = imx_ldb_ch->ldb;
...
//	printk("%s: %ld %ld\n", __func__, clk_get_rate(imx_ldb_ch->clk), clk_get_rate(imx_ldb_ch->clk_sel));
	drm_mode_connector_attach_encoder(&imx_ldb_ch->connector, &imx_ldb_ch->encoder);
	...
	drm_encoder_helper_add(&imx_ldb_ch->encoder, &imx_ldb_encoder_helper_funcs);
	ret = imx_drm_add_encoder(&imx_ldb_ch->encoder, &imx_ldb_ch->imx_drm_encoder, THIS_MODULE);
	if (ret) {
		dev_err(ldb->dev, "adding encoder failed with %d\n", ret);
		return ret;
	}

	drm_connector_helper_add(&imx_ldb_ch->connector, &imx_ldb_connector_helper_funcs);

	ret = imx_drm_add_connector(&imx_ldb_ch->connector, &imx_ldb_ch->imx_drm_connector,
			THIS_MODULE);
	if (ret) {
...
static int __devinit imx_ldb_probe(struct platform_device *pdev)
{
	...
	if (!res)
		return -ENODEV;
	imx_ldb->base = devm_ioremap(&pdev->dev, res->start, resource_size(res));
