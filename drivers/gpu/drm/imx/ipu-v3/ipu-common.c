...
{
	struct ipu_soc *ipu = channel->ipu;
	struct ipu_ch_param *p = ipu_get_cpmem(channel);
	u32 val;
...
int ipu_cpmem_set_fmt(struct ipu_ch_param *cpmem, u32 pixelformat)
{
	switch (pixelformat) {
	case V4L2_PIX_FMT_YUV420:
		ipu_ch_param_set_field(cpmem, IPU_FIELD_PFS, 2);   /* pix format */
		ipu_ch_param_set_field(cpmem, IPU_FIELD_NPB, 63);  /* burst size */
		break;
	case V4L2_PIX_FMT_UYVY:
		ipu_ch_param_set_field(cpmem, IPU_FIELD_BPP, 3);	/* bits/pixel */
		ipu_ch_param_set_field(cpmem, IPU_FIELD_PFS, 0xA);	/* pix format */
		ipu_ch_param_set_field(cpmem, IPU_FIELD_NPB, 31);	/* burst size */
		break;
	case V4L2_PIX_FMT_YUYV:
		ipu_ch_param_set_field(cpmem, IPU_FIELD_BPP, 3);	/* bits/pixel */
		ipu_ch_param_set_field(cpmem, IPU_FIELD_PFS, 0x8);	/* pix format */
		ipu_ch_param_set_field(cpmem, IPU_FIELD_NPB, 31);	/* burst size */
		break;
...
int ipu_cpmem_set_image(struct ipu_ch_param *cpmem, struct ipu_image *image)
{
...
static int ipu_add_client_devices(struct ipu_soc *ipu)
{
	...
	ret |= ipu_add_subdevice_pdata(ipu->dev, "imx-ipuv3-crtc", 0, NULL,
...
static int __devinit ipu_probe(struct platform_device *pdev)
{
	...
	ipu->ic_reg = devm_ioremap(&pdev->dev, ipu_base + IPU_IC_REG_BASE, PAGE_SIZE);
