				...
				unsigned long *prate)
{
	...
	dev_dbg(di->ipu->dev, "%s: inrate: %ld div: 0x%08x outrate: %ld wanted: %ld\n",
...
int ipu_di_init_sync_panel(struct ipu_di *di, struct ipu_di_signal_cfg *sig)
{
	...
	if (ret) {
		dev_err(di->ipu->dev, "setting pixel clock to parent %s failed with %d\n",
		...
		u32 module, struct clk *clk_ipu)
{
	...
	di->clk_name = kasprintf(GFP_KERNEL, "%s_di%d_pixel", dev_name(dev), id);
	if (!di->clk_name)
		return -ENOMEM;
