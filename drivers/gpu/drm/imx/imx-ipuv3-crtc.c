...
static int ipu_get_resources(struct ipu_priv *ipu_priv, struct ipu_crtc *ipu_crtc)
{
	...
	ipu_crtc->di = ipu_di_get(ipu, ipu_crtc->pipe);
		...
		u32 pixfmt)
{
	...
	case DRM_MODE_ENCODER_LVDS:
		ipu_crtc->di_clkflags = IPU_DI_CLKMODE_SYNC | IPU_DI_CLKMODE_EXT;
		break;
...
struct imx_drm_crtc_helper_funcs ipu_crtc_helper_funcs = {
...
static int ipu_crtc_init(struct ipu_priv *ipu_priv, struct ipu_crtc *ipu_crtc, int num)
{
	...
	if (ret) {
		dev_err(ipu_priv->dev, "getting resuorces failed with %d.\n", ret);
		return ret;
	}
	...
	ret = imx_drm_add_crtc(&ipu_crtc->base,
			&ipu_crtc->imx_crtc,
			&ipu_crtc_funcs, &ipu_helper_funcs,
			&ipu_crtc_helper_funcs, THIS_MODULE);
	if (ret) {
		dev_err(ipu_priv->dev, "adding crtc failed with %d.\n", ret);
...
static int __devexit ipu_drm_remove(struct platform_device *pdev)
{
	...
	imx_drm_remove_crtc(ipu_priv->crtc[0].imx_crtc);
	imx_drm_remove_crtc(ipu_priv->crtc[1].imx_crtc);
